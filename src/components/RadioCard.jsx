...
function RadioCard({ radio, isActive, isPlaying, isFavorite, onPlay, onToggleFavorite }) {
  ...
  const inner = (
    <>
      ...
      />
      <div
        className={`relative glass rounded-3xl p-5 flex flex-col gap-4 h-full transition-shadow overflow-hidden ${
      ...
      >
        ...
        <Link to={stationHref} className="flex-1 min-w-0 group/title">
          <h3 className="font-display font-semibold text-base leading-tight truncate group-hover/title:text-white transition-colors">
            {radio.name}
          </h3>
          <p className="text-xs text-white/50 mt-1 uppercase tracking-wider inline-flex items-center gap-1">
            {radio.streamType === 'hls' ? t('card.live_hd') : t('card.live_stream')}
            <Info className="h-3 w-3 opacity-0 group-hover/title:opacity-70 transition-opacity" />
          </p>
        </Link>

        <button
          onClick={() => onPlay(radio)}
          className="relative w-full inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-sm font-semibold text-white overflow-hidden transition-all hover:brightness-110"
          style={{
